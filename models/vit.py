...
def ViT(img_size, num_classes):
    model = VisionTransformer(num_classes, image_size=img_size)
    return model

if __name__ == '__main__':
    x = torch.randn(10,3,224,224).cuda()
    model = VisionTransformer(100).cuda()
    import torchsummary
    torchsummary.summary(model, (3,224,224),128, 'cuda')
    # x = model(x)
    # print(x.shape)